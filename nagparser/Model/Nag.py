...
class Nag(Base):
    ...
    def laststatuschange(self, returntimesincenow=True):
        ...
        """
        lastchange = max(
            self.services, key=lambda x: x.laststatuschange(returntimesincenow=False)
    ...
    def getservicegroups(self, onlyimportant=False):
        ...
        def _getservicegroups(onlyimportant=onlyimportant):
            if onlyimportant:
                servicegroups = NagList(
                    [
                        x
                        for x in self._servicegroups
                        if x.servicegroup_name in self.importantservicegroups
                    ]
                )
            else:
